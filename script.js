...
function SearchResults (city){
    fetch(`${api.base}weather?q=${city}&lang=${api.lang}&units=${api.units}&appid=${api.key}`).then(res => {
        return res.json()
    }).then(response => {
        if (response.cod == "404") {
...
highLow.innerHTML = `${Math.floor(info.main.temp_min)}°/${Math.floor(info.main.temp_max)}°`
